...
function XPerl_Unit_GetHealth(self)
	...
	if RealMobHealth~=nil then  -- DaMaGepy
		if not UnitIsPlayer(partyid) then
			if hpMax>0 and partyid~=nil and RealMobHealth.IsUnitMob(partyid) then 
				--local XUnitName = UnitName(partyid)
				local MH_hp, MH_hpMax = RealMobHealth.GetHealth(partyid)
				if MH_hp~=nil and MH_hpMax~=nil then hp=MH_hp; hpMax=MH_hpMax; end
				--DEFAULT_CHAT_FRAME:AddMessage(XUnitName.." ("..partyid..")      HP: "..hp.."      Max: "..hpMax)
			end
		end
	end
