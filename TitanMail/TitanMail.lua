...
function TitanMail_IncNew()		
	if (TPM_ignorenext) then
		...
		local new = 0; 
		if( TitanGetVar(TITAN_MAIL_ID, "new") ~= nil ) then new = TitanGetVar(TITAN_MAIL_ID, "new"); end 
		local total = 0; 
		if( TitanGetVar(TITAN_MAIL_ID, "total") ~= nil ) then total = TitanGetVar(TITAN_MAIL_ID, "total"); end
		
		if (TPM_checkedmail) then total = GetInboxNumItems() + new;	end
		...
		if (new>0) then
			if (TitanGetVar(TITAN_MAIL_ID, "chat")) then
				DEFAULT_CHAT_FRAME:AddMessage(TITAN_MAIL_CHAT_NEW..": "..new.."  (Total: "..total..")");
			end
